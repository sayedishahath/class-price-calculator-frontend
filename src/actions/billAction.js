...
export const startCreateBill = (bill) =>{
    return async (dispatch) =>{
        try{
            const response = await axios.post('http://localhost:3007/api/bills',bill,{
                headers:{
...
export const startGetMyBill = ()=>{
    return async (dispatch)=>{
        try{
            const response = await axios.get('http://localhost:3007/api/bills/my-bills',{
                headers:{
                    "Authorization":localStorage.getItem('token')
                }
            })
...
export const startShareBill =(selectedBill,selectedUsers)=>{
    return async()=>{
        try{
            const response = await axios.post(`http://localhost:3007/api/bills/${selectedBill}/share`,{userIds:selectedUsers},{
                headers:{
                    "Authorization":localStorage.getItem('token')
                }
            })
...
export const startGetSingleBill = (id)=>{
    return async(dispatch)=>{
        try{
            const response = await axios.get(`http://localhost:3007/api/bills/my-bills/${id}`,{
                headers:{
                    "Authorization":localStorage.getItem('token')
                }
            })
