...
import { addDealDetails,calculatePremiumMinPricePerStudent,calculateincludeGSTAmount,
...
// }
  ...
  useEffect(()=>{
    ...
    dispatch(calculatePremiumMinPricePerStudent())
            ...
            <table className="min-w-full divide-y divide-zinc-200 border border-gray-300">
              <tbody className="divide-y divide-zinc-200" style={{  backgroundColor:"#DCD6F7"}}>
                ...
                <tr>
                  ...
                  <td className={`${selectedBill&&selectedBill.bufferValue < 0 ? 'bg-red-500' : 'bg-green-500'} ${firstTableCellStyle} py-3 ${tableDataStyle}`}>
                    <input
                      readOnly="true"
                      type="text"
                      id="bufferValue"
